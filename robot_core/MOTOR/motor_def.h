...
#ifndef hcan1
#define hcan1 fdhcan1    
#endif
#ifndef hcan2
#define hcan2 fdhcan2    
#endif
#ifndef hcan3
#define hcan3 fdhcan3    
#endif
...
typedef struct {
    ...
    // 此值由积分得出,原点默认为开机处的位置,调整需调用set_motor_mid_position函数
    float rel_angle;
...
typedef struct {
    ...
    float T_ff;                      // 电机期望额外力矩 单位:Nm (ps:GM6020为电压控制
                                     // 当使用GM6020时,该值物理含义为电压值 单位V)
                                     // 当模式为E_MIT时,此值为扭矩限幅
    float P_des;                     // 电机期望位置 单位:rad
    float V_des;                     // 电机期望速度 单位:rad/s
    float output;                    // 电机输出
    float angle_limit_min;           // 电机角度限制最小值 单位:rad
    float angle_limit_max;           // 电机角度限制最大值 单位:rad
