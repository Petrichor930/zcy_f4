...
{
    return (fmodf(((ang1) - (ang2) + (angle_range)*1.5f), (angle_range)) - (angle_range) / 2.f);
}
// 低通滤波
...
uint16_t float_to_uint(float x, float x_min, float x_max, int bits)
{
    ...
    float span = x_max - x_min;
    float offset = x_min;
    uint16_t raw_set = (uint16_t)((x - offset) * ((float)((1 << bits) - 1)) / span);
    return raw_set;
}


float last_in = 0;
float feedback(float in, float time)
{
    float out = 0;
    out = (in - last_in) / time;
    last_in = in;
