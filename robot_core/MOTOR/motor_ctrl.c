...
typedef struct {
    ...
    DM_motor_transmit_msg_t _0[10]; // 使用_0x0A[0],代表ID为0x0B的发出的数据
                                    // ID:1, Master_ID:0xA1, CAN_ID:0x0B
} Tx_msg_canbus_t;
...
HAL_StatusTypeDef set_all_motor_output(motors_type_t motors_type)
{
    ...
    for (uint8_t i = motors_init_index; i < motors_init_index + motors_num; i++) {
        ...
        ///<<< 达妙电机 >>>///
            ...
            switch (motors._[i].type.tx_base_ID) {
            case 0:
                DM_mail = &((Tx_msg_canbus_t *)pbus)->_0[ID];
                break;
            }
            if (DM_mail == NULL) {
                SetMotorErrorCode(motors._[i], MotorError_TransmitMotorIdError);
...
#endif
#if NUM_OF_RM_MOTOR
            if (IS_RM_MOTOR(motors._[i]))
                motors._[i].AUX.EN = false;
#endif
        }
        rslt |= set_all_motor_output(DEFAULT);
        for (uint8_t k = 0xff; k--;)
            ;
    }
    return rslt;
}
